...
function isPositiveNumber(num) {
  if (num > 0) {                     // Перевіряємо, чи число num більше 0
    return true;                     // Якщо так, то num є позитивним, тому повертаємо true
  } 
  else num <= 0;
...
function isEmptyString(str) {
  if (str = ''){                  // Перевіряємо, чи рядок str дорівнює пустому рядку ('')
...
function getUserStatus(age) {
  if (age < 18) {// Перевіряємо вік за допомогою умовних операторів
